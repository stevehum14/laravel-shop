...
class PaymentController extends Controller
{
    public function payByAlipay(Order $order,Request $request)
    {
        ...
        return app('alipay')->web([
            ...
            'subject'=>'支付 Laravel Shop 的订单：'.$order->no, //订单额
        ]);
    }
    // 前端回调页面
...
//        ]);
    ...
    public function payByInstallment(Order $order,Request $request)
    {
        ...
        // 删除同一笔商品订单发起其他的状态是未支付的分期付款，避免同一笔商品订单有多个分期付款
        Installment::query()
        ...
        for ($i = 0;$i < $count;$i++)
        {
            // 最后一起的本金需要总本金减去前面几期的本金
            if($i == $count - 1){
                $base = big_number($order->total_amount)->subtract(big_number($base))
                    ->multiply($count-1);
            }
